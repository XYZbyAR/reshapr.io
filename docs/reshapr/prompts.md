...
- It **must** be bound to a specific reShapr [Service](services-and-artifacts.md) using the [`service.name`](http://service.name) and `service.version` properties which values **must** match an already discovered Service,

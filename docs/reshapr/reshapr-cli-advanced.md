...
The import command allows to push (or have reShapr pull) a new artifact into reShapr. This will allow it to discover a new Service as explained in [Services & Artifacts](services-and-artifacts.md).
...
> 💡 When specifying a remote URL with the `-u` option, it can also be useful to specify a Secret with the `-s, --secret <artifactSecret>` to authorize the access to the remote endpoint. Check the [Secret commands](reshapr-cli-advanced.md) just below to learn how to create them.
> 
...
The import command also allows you to quickly configure and expose the discovered Service using additional flags! If you add the `--be, --backendEndpoint <backendEndpointURL>`  flag to your command, this will create a *default* [Configuration Plan & Exposition](configuration-plan-and-exposition.md) for you, using the *default* gateways.

This exposition can be further configured with the most common options:

- `--apiKey` : Allow the generation of an API key to secure access to an MCP endpoint exposed by gateways.  See the [`reshapr config create` command details](reshapr-cli-advanced.md) below
- `--internalOAuth2` : Allow to secure access to an MCP endpoint using OAuth 2 authorization backed by the reShapr Internal OAuth Identity Provider. See the [`reshapr config create` command details](reshapr-cli-advanced.md) below
- `--bs, --backendSecret <backendSecretId>` : Allow the specification of a Backend Secret to use when exposing an MCP Endpoint on gateways. See [Backend Secrets](security-options-and-secrets.md).
...
Creating a [Secret](security-options-and-secrets.md) just requires an argument that will be its `<name>` . You will also be able to provide a description using the `--description <description>` option. Moreover, the command proposes several options for providing the different elements of your [Secrets](security-options-and-secrets.md).
...
This command proposes advanced options for configuring how your Service to be consumed and enabling different [Security options](security-options-and-secrets.md). A Configuration Plan has a mandatory `name` , so this is the first argument of the command: `reshapr config create <name>` . You can provide a more detailed description of your configuration plan goal using the `-d, --description` option in your command.

...
- It **must** be bound to a specific reShapr [Service](services-and-artifacts.md) using the [`service.name`](http://service.name) and `service.version` properties whose values **must** match an already discovered Service,
...
To fetch its resource content, a `resourceTemplate` **will reuse [the backend endpoint URL](configuration-plan-and-exposition.md) coming from the MCP Server Configuration Plan!**

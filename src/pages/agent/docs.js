...
export default function AgentDocsView() {
  const {siteConfig} = useDocusaurusContext();
  const [content, setContent] = useState('');
  const [loading, setLoading] = useState(true);
  const [section, setSection] = useState('overview');

  // Read ?s= param client-side
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const s = params.get('s');
    if (s && SECTIONS[s]) setSection(s);
  }, []);

  // Hide Docusaurus navbar
  useEffect(() => {
    document.body.dataset.agentView = 'true';
    return () => { delete document.body.dataset.agentView; };
  }, []);

  const sectionMeta = SECTIONS[section] ?? SECTIONS.overview;
  const mdPath = sectionMeta.file;
  const mdUrl = `${siteConfig.url}${mdPath}`;
  const activeHref = `/agent/docs?s=${section}`;
  ...
  return (
    <Layout
      title={`Docs: ${sectionMeta.label} — Agent View`}
      description={`Machine-readable reShapr documentation: ${sectionMeta.label}`}
    >
      <div className={styles.root}>
        <AgentSidebar activeHref={activeHref} />
        <div className={styles.main}>
          <div className={styles.toolbar}>
            <span className={styles.path}>→ {mdPath}</span>
      ...
      <Link to={HUMAN_HREFS[section] ?? '/docs'} className={styles.viewToggle}>

...
/* ─── Page ────────────────────────────────────────────────────────────────── */
export default function AgentBlogView() {
  const {siteConfig} = useDocusaurusContext();
  const [content, setContent] = useState('');
  const [loading, setLoading] = useState(true);
  const mdUrl = `${siteConfig.url}/blog.md`;

  // Hide the Docusaurus navbar while on this page
  useEffect(() => {
  ...
  return (
    <Layout title="Blog — Agent View" description="Machine-readable blog index for AI agents and LLMs.">
      <div className={styles.root}>
        <AgentSidebar activeHref="/agent/blog" />
        <div className={styles.main}>
          <div className={styles.toolbar}>
            <span className={styles.path}>→ /blog.md</span>
      ...
      <Link to="/blog" className={styles.viewToggle}>
